...
Neptune MDM is the management extension for Jellyfin. It keeps preferences consistent across Neptune clients, provides an independent opt-in transport for the current user's private Activity, and gives server administrators tools for remotely managing settings, reusable profiles, announcements, child accounts, and device records.
...
| Private viewing-history continuity | [Activity](/activity) | Synchronizes only the signed-in user's Activity after explicit opt-in; administrators cannot browse it |
