...
Settings Sync keeps supported preferences consistent across Neptune clients. Change a theme on one device and the others update in real time; adjust playback settings on one client and the others pick up the change automatically.
...
The synchronized document includes [Settings Profiles](/personalization/profile-presets), including their names, sparse included settings, override-clearing choices, order, and device-type assignments. The active choice remains local to each physical device, so manually switching profiles on one device does not force every device to use the same profile.

The synchronized document retains profile definitions, assignments, and the device's selection even while a profile is not active. A matching Required assignment is delivered separately as managed policy and keeps the assigned profile active. Its unlocked members remain customizable; only separate member locks are read-only.
...
Settings Sync and managed-policy delivery are available independently of personal Settings Profiles. See the consolidated [MDM availability table](/plugins/mdm#availability) for administrator and recipient requirements.
