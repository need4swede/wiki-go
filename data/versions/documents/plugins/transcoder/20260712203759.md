...
Transcoder is policy-only in nature: it serves the administrator's profile ladder to Neptune, and Neptune asks Jellyfin's existing transcoder to create the selected offline copy. The plugin does not run a separate encoder or alter normal streaming playback.
