...
Suite plugins are free, include useful defaults, and need no API keys. Indexers and MDM are available today; Transcoder and Studio are implemented but held from the public plugin catalog until their client rollouts are ready.
...
| [Studio](/plugins/studio) | UI customization, beginning with visual Top Shelf design (coming soon) |
...
| **Neptune Studio** | Provides a server-backed UI customization system; its first integration is a visual editor and renderer for Banners and Cinematic Top Shelf designs |
...
Transcoder and Studio do not appear in the public catalog yet. When they are released, the same suite installer and repository will pick them up; no separate repository is required.
